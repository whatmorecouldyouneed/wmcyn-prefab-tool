...
program
    .action(async () => {
    console.log(chalk_1.default.blue('🚀 WMCYN Project Initializer'));
    // github auth prompt
    const { githubToken } = await (0, inquirer_1.prompt)([
        {
            type: 'password',
            name: 'githubToken',
    ...
    const spinner = (0, ora_1.default)('Verifying GitHub access...').start();
    const githubService = new github_1.GitHubService(githubToken);
    try {
        ...
        console.error(chalk_1.default.red('Error: You must be a member of the whatmorecouldyouneed organization'));
        process.exit(1);
    }
    // project metadata prompts
    const { projectName, projectType } = await (0, inquirer_1.prompt)([
        {
            type: 'input',
            name: 'projectName',
    ...
    try {
        ...
        console.error(chalk_1.default.red(`Error: Could not create repository: ${error.message}`));
        process.exit(1);
    }
    ...
    if (projectType === 'nextjs') {
        const { useShopify, useFirebase, useTesting } = await (0, inquirer_1.prompt)([
            {
                type: 'confirm',
                name: 'useShopify',
        ...
        if (useShopify) {
            const { shopifyDomain, shopifyToken } = await (0, inquirer_1.prompt)([
                {
                    type: 'input',
                    name: 'shopifyDomain',
        ...
        if (useFirebase) {
            const firebaseConfig = await (0, inquirer_1.prompt)([
                {
                    type: 'input',
                    name: 'apiKey',
        ...
        const { hasDomain } = await (0, inquirer_1.prompt)([
            {
                type: 'confirm',
                name: 'hasDomain',
    ...
    if (projectType === 'html') {
        const { useShopify, useFirebase } = await (0, inquirer_1.prompt)([
            {
                type: 'confirm',
                name: 'useShopify',
                message: 'Do you want to integrate Shopify?',
    ...
    try {
        const scaffoldService = new scaffold_1.ScaffoldService(path_1.default.join(__dirname, 'templates'), path_1.default.join(process.cwd(), projectName));
        await scaffoldService.scaffoldProject(options);
        spinner.succeed('Project scaffolded successfully');
    }
    catch (error) {
        spinner.fail('Failed to scaffold project');
        console.error(chalk_1.default.red('Error: Could not scaffold project'));
        process.exit(1);
    }
    // final confirmation
